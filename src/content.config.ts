...
const moments = defineCollection({
  ...
  schema: z.object({
     ...
     * (\n) נשמרת בכרטיס. **חובה** מאז 2026-09-25: בזמן ההוספה השדה היה
     * אופציונלי (אותו דפוס שהכניס את title); כל 109 קיבלו אותו, ומעכשיו רגע
     * בלי שורה לדרך מכשיל את הבנייה.
     */
...
const momentsEs = defineCollection({
  ...
  schema: z.object({
    ...
    title: z.string().min(1),
    // "Para el camino" – תרגום של השדה העברי, אותם כללים (עד שתי שורות).
    // חובה, כמו בעברית.
    handle: z.string().min(1),
  }),
});
